...
test_that("manual_upload() function works as expected", {
  ...
  psychinfo <- manual_upload(
    "search_manual/psychinfo.ris",
    source = "psychinfo"
  )
  ...
  expect_true(
    is.data.frame(psychinfo),
    info = "psychinfo should be a data frame"
  )
...
test_that("manual_upload() function handles errors", {
  ...
  expect_error(manual_upload(
    "search_manual/wos.bib",
